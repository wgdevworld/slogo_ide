/**
 ...
 The IfElse class implements the Executable interface and represents the "If" command in the SLogo API.
...
public class IfElse extends Commands implements Executable {

    private double value;

    /**
     * Executes the "IfElse" command on a turtle.
     * @param paramsList the list of parameters to the "IfElse" command
    ...
    public double setBehavior(ArrayList<Double> paramsList, Turtle turtle, Stack<ArrayList<String>> listStack, List<TurtleObserver> observers) throws Exception {
        ...
        ArrayList<String> elseStack = listStack.pop();
        elseStack.remove(0);
        ...
        if (ifbool) {
            ...
            value = parser.parseTokenizedInput(elseStack).peek();
        }
        return value;
    }
    ...
    /**
     * Prepares the "IfElse" command for execution.
     * @param commandString the string representing the "IfElse" command
     */
    @Override
