...
class CosineTest {
  ...
  void testSetBehavior(double degrees) {
    ...
    assertEquals(Math.cos((degrees * Math.PI / 180)), cos.setBehavior(paramList, turtle, listStack, parser.getObservers()));
  }
  ...
  void testExecute(double degrees) {
    ...
    assertEquals(Math.cos(degrees * Math.PI / 180), cos.execute(turtle));
  }
}